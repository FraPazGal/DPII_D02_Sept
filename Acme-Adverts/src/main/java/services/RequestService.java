...
public class RequestService {
	...
	public Request findOne(final int id) {
		...
		final Request r = this.requestRepository.findOne(id);
		Assert.isTrue(r.getCustomer().getId() == principal.getId() || r.getPack().getManager().getId() == principal.getId());
		return r;
	}
	...
	public Request reconstruct(final Request requestF, final BindingResult binding) {
		...
		if (requestF.getId() != 0)
			try {
				Assert.isTrue(requestF.getCommentsManager() != null || requestF.getCommentsManager() != "", "request.commentManager.error");
	...
	public Collection<Request> getList(final int id) {
		...
		requests = this.requestRepository.findByPackageAndOwner(principal.getId(), id);
		return requests;
	}
	...
	public void rejected(final Request requestF) {
		this.checkRequest();
		final Actor principal = this.actorService.findByPrincipal();
		Assert.notNull(principal);
		final Request request = this.findOne(requestF.getId());
		Assert.isTrue(this.actorService.checkAuthority(principal, "MANAGER"));
		Assert.notNull(request);
		Assert.isTrue(request.getId() != 0);
		Assert.isTrue(request.getPack().getManager().equals(principal));
		...
		if (!requestF.getCommentsManager().isEmpty())
			request.setCommentsManager(requestF.getCommentsManager());
		this.requestRepository.save(request);
	}
